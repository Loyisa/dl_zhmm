...
if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'yz') {
	// 开始检查验证码
	loyisa_zhmm_recaptcha_check();
	$mail = !empty($_POST['mail']) ? (checkMail($_POST['mail']) ? $_POST['mail'] : ReDirect('index.php?pub_plugin=loyisa_zhmm&error_msg=' . urlencode('邮箱地址不合法，请重新输入'))) : ReDirect('index.php?pub_plugin=loyisa_zhmm&error_msg=' . urlencode('邮箱地址为空，请重新输入'));
	$cx = $m->query("SELECT * FROM  `" . DB_NAME . "`.`" . DB_PREFIX . "users` WHERE email = '{$mail}' LIMIT 1");
	$p = $m->fetch_array($cx);
	if ($p == "") {
		ReDirect(SYSTEM_URL . 'index.php?pub_plugin=loyisa_zhmm&error_msg=' . urlencode('错误：未能在本站找到持有该邮箱的用户！'));
		die();
	}
	$pw = sha1(md5(EncodePwd($p['pw'] . date('Ymd') . SYSTEM_NAME . SYSTEM_VER . SYSTEM_URL)));
	$title = strip_tags(SYSTEM_NAME) . " - 找回密码";
	$text = "请点击以下链接重置密码：" . SYSTEM_URL . "index.php?pub_plugin=loyisa_zhmm" . '&page=yjqr' . '&email=' . base64_encode($mail) . '&key=' . $pw;
	$x = misc::mail($mail, $title, $text);
	if ($x != true) {
		ReDirect(SYSTEM_URL . 'index.php?pub_plugin=loyisa_zhmm&error_msg=' . urlencode('错误：找回密码邮件发送失败，请检查邮件相关设置！'));
	} else {
		ReDirect(SYSTEM_URL . 'index.php?pub_plugin=loyisa_zhmm&success_msg=' . urlencode('成功：邮件发送成功，请登录邮箱按照提示操作！'));
	}
}
?>
<?php
if (isset($_GET['xg'])) {
	...
	$pw = sha1(md5(EncodePwd($p['pw'] . date('Ymd') . SYSTEM_NAME . SYSTEM_VER . SYSTEM_URL)));
	if ($pw != $key) {
		ReDirect(SYSTEM_URL . 'index.php?pub_plugin=loyisa_zhmm&error_msg=' . urlencode('错误：该链接失效或者不归您所拥有，修改密码失败！'));
		die();
	} else {
...
?>
		<div class="panel panel-success" style="margin:1% 1% 1% 1%;">
			...
			<div style="margin:0% 5% 5% 5%;">
				...
				<b>请输入您新密码</b><br /><br />
				<form name="f" method="post" action="<?php echo SYSTEM_URL . 'index.php?pub_plugin=loyisa_zhmm&xg&email=' . base64_encode($email) . '&key=' . $key ?>">
					...
					<p>插件原作者：<a href="http://www.tbsign.cn" target="_blank">D丶L</a>&nbsp;&nbsp;由<a href="https://loyisa.cn" target="_blank">Loyisa</a>进行二次开发&nbsp;&nbsp;\\&nbsp;&nbsp;程序作者：<a href="http://zhizhe8.net" target="_blank">无名智者</a></p>
...
<div class="panel panel-success" style="margin:1% 1% 1% 1%;">
	...
	<div style="margin:0% 5% 5% 5%;">
		...
		<?php if (isset($_GET['error_msg'])) : ?><div class="alert alert-danger alert-dismissable">
				...
				<?php echo strip_tags(urldecode($_GET['error_msg'])); ?>
			</div><?php endif; ?>
		<?php if (isset($_GET['success_msg'])) : ?><div class="alert alert-success alert-dismissable">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<?php echo strip_tags(urldecode($_GET['success_msg'])); ?>
			</div><?php endif; ?>
		<form name="f" method="post" action="<?php echo SYSTEM_URL ?>index.php?pub_plugin=loyisa_zhmm&page=yz&zhmm">
			<?php
			if (isset($_GET['success_msg'])) {
				echo '';
			} else {
